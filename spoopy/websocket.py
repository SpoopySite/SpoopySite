...
@bp.websocket("/ws")
async def ws_spoopy(request: sanic.request.Request, ws: WebsocketImplProtocol):
    with start_transaction(op="task", name="Websocket") as transaction:
        url = await ws.recv()
        ...
        for url in url_pool:
            ...
            try:
                data = await get_check_website(url, request.app.session, request.app.db, request.app.fish, transaction)
